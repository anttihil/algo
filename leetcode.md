...
Time complexity = O(nlog(n)) This is based on Firefox using merge sort for .sort() implementation.

Space complexity = O(n) We need two arrays that are length s and t, plus some constant more for split.
...
Space Complexity: O(n) (stack takes max n/2 which entails O(n))
...
The idea: One way to think about the problem is that each contiguous array has a center from which we can start to sum up the numbers and decide whether to include them into the array or not.
